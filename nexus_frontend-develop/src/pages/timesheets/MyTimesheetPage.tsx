...
function hoursStyle(day: DayInfo): React.CSSProperties {
  ...
  if (day.total_hours >= day.capacity) return { color: "#22c55e" }; // exactly met — green
  return {};
}
...
const isAdmin = useAuthStore((s) =>
  ...
  // ── Export handler ─────────────────────────────────────────────────────────
  const handleExport = () => {
    // timesheetApi may not expose exportUrl in some builds; fall back to a direct endpoint
    if (typeof (timesheetApi as any).exportUrl === "function") {
      const url = (timesheetApi as any).exportUrl(weekStart, viewingOther ? selectedEmployee : undefined);
      window.open(`/api/v1${url}`, "_blank");
      return;
    }

    // Fallback: construct direct export URL
    const params = new URLSearchParams();
    params.set("week_start", weekStart);
    if (viewingOther && selectedEmployee) params.set("employee_id", selectedEmployee);
    window.open(`/api/v1/timesheets/export?${params.toString()}`, "_blank");
  };
                    ...
                    <tr
                      ...
                      style={{
                        background: d.is_holiday
                          ? "#fef9c3"
                          : d.is_on_leave
                            ? d.leave_is_paid === false ? "#fee2e2" : "#ede9fe"
                            : "transparent",
                        borderBottom: "1px solid #f3f4f6",
                      }}
                    >
                      <td style={{ padding: "7px 10px", fontWeight: 500 }}>{d.day_name.slice(0, 3)}</td>
                      <td style={{ padding: "7px 10px" }}>{dayjs(d.date).format("DD MMM")}</td>
                      <td style={{ padding: "7px 10px", color: "#16a34a" }}>
                        {d.check_in ?? <span style={{ color: "#9ca3af" }}>—</span>}
                      </td>
                      <td style={{ padding: "7px 10px", color: "#dc2626" }}>
                        {d.check_out ?? <span style={{ color: "#9ca3af" }}>—</span>}
                      </td>
                      ...
                      <td style={{ padding: "7px 10px" }}>
                        <span style={{ fontWeight: 600, color: mismatch ? "#d97706" : "inherit" }}>
                          {d.is_holiday ? "0h" : `${d.total_hours}h`}
                        </span>
                        {mismatch && <span style={{ color: "#d97706", marginLeft: 4 }}>⚠</span>}
                      </td>
