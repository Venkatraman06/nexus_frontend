...
export default function ReportingTimesheetPage() {
  ...
  const aggregateColumns = [
    { title: "Employee",   dataIndex: "employee_name", key: "name", width: 160 },
    ...
    },
    {
      ...
      render: (v: number) => (
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          />
          <span style={{ fontSize: 12, color: utilizationColor(v), fontWeight: 600 }}>{v}%</span>
        </div>
      ),
          ...
          <Tooltip title="Export filtered list to CSV">
            <Button
              ...
              onClick={() => {
                ...
                const url = `/api/v1/timesheets/reporting/export${exportParams.toString() ? `?${exportParams}` : ""}`;
