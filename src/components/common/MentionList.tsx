...
const MentionList = forwardRef<MentionListHandle, MentionListProps>(({ items, command }, ref) => {
  ...
  if (items.length === 0) {
    return (
      <div style={{ padding: "8px 12px", fontSize: 13, color: "var(--pmt-text-2, #6b7280)" }}>
        No matching people
      </div>
    );
  }
      ...
      {items.map((item, index) => (
        <button
          ...
          style={{
            ...
            background: index === selectedIndex ? "var(--pmt-primary-bg, #e6f4ff)" : "transparent",
            fontSize: 13,
          }}
        >
          <AssigneeAvatar name={item.label} src={item.avatarSrc} size={22} />
          <span style={{ color: "var(--pmt-text, #1f2937)" }}>{item.label}</span>
        </button>
      ))}
    </div>
  );
