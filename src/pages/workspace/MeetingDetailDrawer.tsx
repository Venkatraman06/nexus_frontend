...
import { MeetingItem, MEETING_PRIORITIES, meetingApi } from "@/services/meetings";
...
export default function MeetingDetailDrawer({
  ...
  open: boolean;
  onClose: () => void;
  onEdit: (item: MeetingItem) => void;
  onDone: (item: MeetingItem) => void;
  onDelete: (id: string) => void;
  canUpdate: boolean;
  canDelete: boolean;
  canTransition: boolean;
}) {
  const [localItem, setLocalItem] = useState<MeetingItem | null>(null);

  useEffect(() => { setLocalItem(propItem); }, [propItem]);

  const { data: remoteItem } = useQuery({
    queryKey: ["meeting-detail", propItem?.id],
    queryFn: () => (propItem?.id ? meetingApi.get(propItem.id) : null),
  ...
  const addCommentMutation = useMutation({
    mutationFn: (newComment: string) => {
      ...
      return meetingApi.update(item.id, { comments: finalComments });
    },
    onSuccess: (data) => {
      setCommentText("");
      message.success("Comment added");
      setLocalItem((prev) => prev ? { ...prev, comments: data.comments } : null);
      qc.invalidateQueries({ queryKey: ["meeting-detail", item?.id] });
  ...
  const pri = MEETING_PRIORITIES.find((p) => p.value === item.priority) ?? MEETING_PRIORITIES[2];
  ...
  return (
    <Drawer
      title={<span style={{ fontWeight: 600, fontSize: 15 }}>Meeting details</span>}
      open={open}
      onClose={onClose}
      width={420}
      styles={{
      ...
      footer={
        <Space wrap style={{ padding: "4px 0" }}>
          ...
          {canDelete && (
            <Popconfirm title="Delete this meeting?" onConfirm={() => { onDelete(item.id); onClose(); }}>
    ...
    >
      ...
      <Space wrap size={[8, 8]} style={{ marginBottom: 4 }}>
        ...
        <Tag
          style={{
            background: item.meeting_mode === "ONLINE" ? "#0284c7" : item.meeting_mode === "OFFLINE" ? "#0d9488" : "#2563eb",
